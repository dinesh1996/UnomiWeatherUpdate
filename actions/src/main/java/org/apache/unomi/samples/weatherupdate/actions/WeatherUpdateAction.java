...
package org.apache.unomi.samples.weatherupdate.actions;
...
public class WeatherUpdateAction implements ActionExecutor {
    ...
    public int execute(Action action, Event event) {
        ...
        //String apiKey = ((CustomItem) event.getTarget()).getProperties().get("apiKeyWeather").toString();
        ...
        if (sessionProperties.containsKey("location")) {
            ...
            HttpGet httpGet = new HttpGet("http://api.openweathermap.org/data/2.5/weather?lat=" +
                    location.get("lat") + "&lon=" + location.get("lon") + "&appid=" +
                    "3e66ec6001684dredfgad10724vdbddaf547e6");
            ...
            if (jsonNode.has("cod")) {
                if (jsonNode.get("cod").asText().equals("200")) {
                    ...
                    if (temperature != null) {
                        filedPropreties(session, "weatherTemperature", temperature);
                    }
                    if (weatherLike != null) {
                        filedPropreties(session, "weatherLike", weatherLike);
                    }
                    if (windDirection != null) {
                        filedPropreties(session, "weatherWindDirection", windDirection);
                    }
                    if (windSpeed != null) {
                        filedPropreties(session, "weatherWindSpeed", windSpeed);
                    }
